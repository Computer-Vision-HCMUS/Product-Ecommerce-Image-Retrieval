...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.video_layer=nn.ModuleList(
            [copy.deepcopy(v_layer) for _ in range(config.v_num_hidden_layers)]
        )

        self.audio_layer=nn.ModuleList(
            [copy.deepcopy(v_layer) for _ in range(config.v_num_hidden_layers)]
        )
        ...
        self.pv_pooler = BertTextPooler(config)
        self.v_pooler = BertImagePooler(config)
        self.video_pooler=BertImagePooler(config)
        self.audio_pooler = BertImagePooler(config)
    ...
    def graph_construct(self,modality_feature):  # 5*B*H
        ...
        for b_i in range(modality_feature.shape[1]):
            ...
            group_sim = F.cosine_similarity(group_modality.unsqueeze(1), group_modality.unsqueeze(0), dim=2)
            batch_modality_graph = batch_modality_graph + group_sim

        batch_modality_graph = batch_modality_graph / modality_feature.shape[0]
        revised_modality_graph = batch_modality_graph * F.softmax(batch_modality_graph, dim=1)
        batch_modality_graph = F.normalize(revised_modality_graph, dim=0)
    ...
    def graph_construct_per_sample(self,modality_feature):  # 5*B*H
        ...
        for b_i in range(modality_feature.shape[1]):
            group_modality = torch.reshape(modality_feature[:, b_i, :],
                                           (modality_feature.shape[0], modality_feature.shape[2]))
            group_sim = F.cosine_similarity(group_modality.unsqueeze(1), group_modality.unsqueeze(0), dim=2)
            revised_modality_graph = group_sim * F.softmax(group_sim, dim=-1)
            group_sim = F.normalize(revised_modality_graph, dim=-1)
            batch_modality_graph[b_i,:,:] = group_sim
    ...
    def forward(
            ...
            audio_embedding,
            txt_attention_mask,
            pv_txt_attention_mask,
            image_attention_mask,
            video_attention_mask,
            audio_attention_mask,
            co_attention_mask=None,
            co_video_text_attention_mask=None,
        ...
        all_encoder_layers_pv = []
        all_encoder_layers_v = []
        all_encoder_layers_video=[]
        all_encoder_layers_audio=[]

        all_attention_mask_t = []
        all_attention_mask_pv = []
        all_attnetion_mask_v = []
        all_attention_mask_video=[]
        all_attention_mask_c = []
        all_attention_mask_c_video_text=[]
        ...
        for idx in range(0, self.v_num_hidden_layers):
            video_embedding, video_attention_probs = self.v_layer[idx](video_embedding, video_attention_mask)

        for idx in range(0, self.v_num_hidden_layers):
            txt_embedding, txt_attention_probs = self.layer[idx](txt_embedding, txt_attention_mask)

        for idx in range(0, self.v_num_hidden_layers):
            audio_embedding,audio_attention_probs=self.audio_layer[idx](audio_embedding,audio_attention_mask)

        for idx in range(0, self.num_hidden_layers):
            pv_txt_embedding, pv_txt_attention_probs =self.layer[idx](pv_txt_embedding,pv_txt_attention_mask)

        pooled_feature_v = self.v_pooler(image_embedding)
        pooled_feature_t = self.t_pooler(txt_embedding)
        pooled_feature_pv = self.pv_pooler(pv_txt_embedding)
        pooled_feature_video=self.video_pooler(video_embedding)
        pooled_feature_audio=self.audio_pooler(audio_embedding)

        # dyctr
        concat_feature=torch.cat((pooled_feature_t.unsqueeze(0),
                                  pooled_feature_v.unsqueeze(0),
                                  pooled_feature_pv.unsqueeze(0),
                                  pooled_feature_video.unsqueeze(0),
                                  pooled_feature_audio.unsqueeze(0)),dim=0)
        ...
        all_features=[pooled_feature_t,
                      pooled_feature_v,
                      pooled_feature_pv,
                      pooled_feature_video,
                      pooled_feature_audio]
        for i in range(len(all_features)):
            for j in range(i+1,len(all_features)):
                ...
                logits = torch.cat((graph[:,i,j].reshape(len(all_features[0]),1), graph[:,i,j].reshape(len(all_features[0]),1))).expand(logits.shape[0], logits.shape[1]).mul(logits)
                CLR_loss += self.loss_fct(logits, labels)
                #CLR_loss += graph[i][j]*self.loss_fct(logits, labels)
                cnt+=1
        CLR_loss/=cnt
        ...
        if not output_all_encoded_layers:
            ...
            all_encoder_layers_pv.append(pv_txt_embedding)
            all_encoder_layers_v.append(image_embedding)
            all_encoder_layers_video.append(video_embedding)
            all_encoder_layers_audio.append(audio_embedding)

        return all_encoder_layers_t, all_encoder_layers_pv, \
               all_encoder_layers_v,all_encoder_layers_video,\
               all_encoder_layers_audio,\
...
class BertPreTrainingHeads_woITM(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        ...
        self.predictions = BertLMPredictionHead(config, bert_model_embedding_weights)
        self.imagePredictions = BertImagePredictionHead(config)
        self.videoPredictions = BertVideoPredictionHead(config)
    ...
    def forward(
        self, sequence_output_t, sequence_output_pv, sequence_output_v,sequence_output_video,sequence_output_audio
    ):
        prediction_scores_t = self.predictions(sequence_output_t)
        prediction_scores_pv = self.predictions(sequence_output_pv)
        prediction_scores_v = self.imagePredictions(sequence_output_v)
        prediction_scores_video=self.videoPredictions(sequence_output_video)
        prediction_scores_audio = self.audioPredictions(sequence_output_audio)

        return prediction_scores_t, prediction_scores_pv, prediction_scores_v,prediction_scores_video,prediction_scores_audio
...
class BertVideoPredictionHead(nn.Module):
    def __init__(self, config):
        super(BertVideoPredictionHead, self).__init__()
        self.transform = BertImgPredictionHeadTransform(config)

        # The output weights are the same as the input embeddings, but there is
        # an output-only bias for each token.
        self.decoder = nn.Linear(config.v_hidden_size, config.video_target_size)
...
class BertAudioPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        # The output weights are the same as the input embeddings, but there is
        # an output-only bias for each token.
        self.decoder = nn.Linear(config.v_hidden_size, config.audio_target_size)
...
class BertModel(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.pv_embeddings = BertEmbeddings(config)
        self.v_embeddings = BertImageEmbeddings(config)
        self.video_embeddings=BertVideoEmbeddings(config)
    ...
    def _get_cross_output(self, sequence_output, pv_sequence_output, visual_output, video_output,
                          audio_output,attention_mask, pv_attention_mask, image_mask,video_mask,audio_mask):
        concat_features = torch.cat((sequence_output, pv_sequence_output,visual_output, video_output,audio_output),
                                    dim=1)  # concatnate tokens and frames
        concat_mask = torch.cat((attention_mask, pv_attention_mask, image_mask, video_mask,audio_mask), dim=1)
        text_type_ = torch.zeros_like(attention_mask)
        pv_type_=torch.zeros_like(pv_attention_mask)
        image_type_ = torch.ones_like(image_mask)
        video_type_ = torch.ones_like(video_mask)
        audio_type_ = torch.ones_like(audio_mask)
        concat_type = torch.cat((text_type_, pv_type_,image_type_, video_type_,audio_type_), dim=1)
        cross_output, pooled_output = self.cross_encoder(concat_features, concat_type, concat_mask)

        sequence_output_t, sequence_output_pv, \
        sequence_output_v,sequence_output_video, sequence_output_audio = torch.split(cross_output,
                                                             [attention_mask.size(-1),
                                                              pv_attention_mask.size(-1),
                                                              image_mask.size(-1),
                                                              video_mask.size(-1),
                                                              audio_mask.size(-1)], dim=1)
        # pdb.set_trace()
        return sequence_output_t,sequence_output_pv,sequence_output_v,sequence_output_video,sequence_output_audio
    ...
    def forward(
            ...
            image_loc,
            token_type_ids=None,
            pv_token_type_ids=None,
            attention_mask=None,
            pv_attention_mask=None,
        ...
        if pv_attention_mask is None:
            pv_attention_mask =torch.ones_like(pv_input_txt)
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(input_txt)
        if pv_token_type_ids is None:
            pv_token_type_ids=torch.zeros_like(pv_input_txt)
        ...
        if video_attention_mask is None:
            video_attention_mask = torch.ones(
                input_videos.size(0), input_videos.size(1)
            ).type_as(input_txt)
        if audio_attention_mask is None:
            audio_attention_mask = torch.ones(
                input_audios.size(0), input_audios.size(1)
            ).type_as(input_txt)

        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
        extended_pv_attention_mask=pv_attention_mask.unsqueeze(1).unsqueeze(2)
        extended_image_attention_mask = image_attention_mask.unsqueeze(1).unsqueeze(2)
        extended_video_attention_mask = video_attention_mask.unsqueeze(1).unsqueeze(2)
        extended_audio_attention_mask = audio_attention_mask.unsqueeze(1).unsqueeze(2)

        extended_attention_mask = extended_attention_mask.to(dtype=torch.float32)
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0

        extended_pv_attention_mask = extended_pv_attention_mask.to(dtype=torch.float32)
        extended_pv_attention_mask = (1.0 - extended_pv_attention_mask) * -10000.0

        extended_image_attention_mask = extended_image_attention_mask.to(dtype=torch.float32)
        extended_image_attention_mask = (1.0 - extended_image_attention_mask) * -10000.0

        extended_video_attention_mask = extended_video_attention_mask.to(dtype=torch.float32)
        extended_video_attention_mask = (1.0 - extended_video_attention_mask) * -10000.0

        extended_audio_attention_mask = extended_audio_attention_mask.to(dtype=torch.float32)
        ...
        co_video_text_attention_mask = torch.zeros(input_txt.size(0), input_videos.size(1), input_txt.size(1)).type_as(
            extended_image_attention_mask)

        extended_co_attention_mask = co_attention_mask.unsqueeze(1)
        extended_co_video_text_attention_mask = co_video_text_attention_mask.unsqueeze(1)

        # extended_co_attention_mask = co_attention_mask.unsqueeze(-1)
        extended_co_attention_mask = extended_co_attention_mask * 5.0
        extended_co_video_text_attention_mask = extended_co_video_text_attention_mask * 5.0
        extended_co_attention_mask = extended_co_attention_mask.to(dtype=torch.float32)
        extended_co_video_text_attention_mask = extended_co_video_text_attention_mask.to(dtype=torch.float32)

        embedding_output = self.embeddings(input_txt, token_type_ids)
        pv_embedding_output = self.embeddings(pv_input_txt,pv_token_type_ids)
        v_embedding_output = self.v_embeddings(input_imgs, image_loc)
        video_embedding_output=self.video_embeddings(input_videos)
        audio_embedding_output=self.audio_embedding(input_audios)

        # sep encoder
        encoded_layers_t, encoded_layers_pv,encoded_layers_v, encoded_layers_video,\
        encoded_layers_audio, CLR_loss, all_attention_mask = self.encoder(
            embedding_output,
            pv_embedding_output,
            v_embedding_output,
            video_embedding_output,
            audio_embedding_output,
            extended_attention_mask,
            extended_pv_attention_mask,
            extended_image_attention_mask,
            extended_video_attention_mask,
            extended_audio_attention_mask,
            extended_co_attention_mask,
            extended_co_video_text_attention_mask,
            output_all_encoded_layers=output_all_encoded_layers,
            output_all_attention_masks=output_all_attention_masks,
        )
        sequence_output_t = encoded_layers_t[-1]
        sequence_output_pv=encoded_layers_pv[-1]
        sequence_output_v = encoded_layers_v[-1]
        sequence_output_video=encoded_layers_video[-1]
        sequence_output_audio=encoded_layers_audio[-1]

        # co encoder
        sequence_output_t, sequence_output_pv, sequence_output_v, \
        sequence_output_video,sequence_output_audio=self._get_cross_output(
            sequence_output_t,sequence_output_pv,sequence_output_v,
            sequence_output_video,sequence_output_audio,
            attention_mask,pv_attention_mask,
            image_attention_mask,video_attention_mask,
            audio_attention_mask
        )

        pooled_output_t=sequence_output_t[:,0]
        pooled_output_pv=sequence_output_pv[:,0]
        pooled_output_v=sequence_output_v[:,0]
        pooled_output_video=sequence_output_video[:,0]
        pooled_output_audio=sequence_output_audio[:,0]


        if not output_all_encoded_layers:
            encoded_layers_t = encoded_layers_t[-1]
            encoded_layers_pv = encoded_layers_pv[-1]
            encoded_layers_v = encoded_layers_v[-1]
            encoded_layers_video=encoded_layers_video[-1]
            encoded_layers_audio=encoded_layers_audio[-1]

        return encoded_layers_t, encoded_layers_pv, \
               encoded_layers_v, encoded_layers_video, \
               encoded_layers_audio, \
               pooled_output_t, pooled_output_pv, \
               pooled_output_v, pooled_output_video, \
               pooled_output_audio, \
...
class BertAudioEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.audio_embeddings = nn.Linear(config.audio_feature_size, config.v_hidden_size)
        self.position_embeddings = nn.Embedding(
            config.max_audio_len, config.hidden_size
        )
        self.LayerNorm = BertLayerNorm(config.v_hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, input_audios):
        audio_embeddings = self.audio_embeddings(input_audios)

        seq_length = audio_embeddings.size(1)
        position_ids = torch.arange(
            seq_length, dtype=torch.long, device=input_audios.device
        )
        # position_ids = position_ids.unsqueeze(0).expand_as(input_ids)

        position_embeddings = self.position_embeddings(position_ids)
        embeddings = self.LayerNorm(audio_embeddings + position_embeddings)
        embeddings = self.dropout(embeddings)
        return embeddings
...
class BertForMultiModalPreTraining(BertPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output_t, sequence_output_pv,sequence_output_v,\
        sequence_output_video, sequence_output_audio, \
        pooled_output_t, pooled_output_pv, pooled_output_v, pooled_output_video,\
        pooled_output_audio, CLR_loss, all_attention_mask = self.bert(
            input_ids, # 1 x 30
            pv_input_ids,
        ...
        prediction_scores_t, prediction_scores_pv, \
        prediction_scores_v, prediction_scores_video,\
        prediction_scores_audio = self.cls(
            sequence_output_t, sequence_output_pv,
            sequence_output_v,sequence_output_video,
            sequence_output_audio
        )

        concat_feature=torch.cat((pooled_output_t.unsqueeze(0),
                                  pooled_output_v.unsqueeze(0),
                                  pooled_output_pv.unsqueeze(0),
                                  pooled_output_video.unsqueeze(0),
                                  pooled_output_audio.unsqueeze(0)),dim=0)

        graph,modality_weight=self.graph_construct_per_sample(concat_feature)


        prediction_scores_v = prediction_scores_v[:, 1:]
        prediction_scores_video=prediction_scores_video[:,1:]
        prediction_scores_audio=prediction_scores_audio[:,1:]
        

        prediction_scores_t = modality_weight[:,0].unsqueeze(1).unsqueeze(1).expand(modality_weight.shape[0],prediction_scores_t.shape[1], prediction_scores_t.shape[2]).mul(prediction_scores_t)
        prediction_scores_v = modality_weight[:,1].unsqueeze(1).unsqueeze(1).expand(modality_weight.shape[0],prediction_scores_v.shape[1], prediction_scores_v.shape[2]).mul(prediction_scores_v)
        prediction_scores_pv = modality_weight[:, 2].unsqueeze(1).unsqueeze(1).expand(modality_weight.shape[0],
                                                                                      prediction_scores_pv.shape[1],
                                                                                      prediction_scores_pv.shape[2]).mul(prediction_scores_pv)
        prediction_scores_video = modality_weight[:,3].unsqueeze(1).unsqueeze(1).expand(modality_weight.shape[0],prediction_scores_video.shape[1], prediction_scores_video.shape[2]).mul(prediction_scores_video)
        prediction_scores_audio = modality_weight[:,4].unsqueeze(1).unsqueeze(1).expand(modality_weight.shape[0],prediction_scores_audio.shape[1], prediction_scores_audio.shape[2]).mul(prediction_scores_audio)
        ...
        video_loss = self.vis_criterion(prediction_scores_video, video_target)
        # 1 x 36 x 1601
        masked_video_loss = torch.sum(
            video_loss * (video_label == 1).unsqueeze(2).float()
        ) / max(torch.sum((video_label == 1).unsqueeze(2).expand_as(video_loss)), 1)

        audio_loss = self.vis_criterion(prediction_scores_audio, audio_target)
        ...
        if return_features:
            return masked_lm_loss.unsqueeze(0),\
                   ...
                   masked_video_loss.unsqueeze(0), \
                   masked_audio_loss.unsqueeze(0), \
                   CLR_loss.unsqueeze(0),\
                   pooled_output_t,\
                   pooled_output_pv,\
                   pooled_output_v,\
                   pooled_output_video,\
                   pooled_output_audio,graph,modality_weight
        else:
            return masked_lm_loss.unsqueeze(0), \
                   masked_em_loss.unsqueeze(0), \
                   masked_img_loss.unsqueeze(0), \
                   masked_video_loss.unsqueeze(0), \
                   masked_audio_loss.unsqueeze(0), \
                   CLR_loss.unsqueeze(0) # right,all_num  # TODO: + pooled_output_t,pooled_output_v



class Capture_ITPV_ForClassification(BertPreTrainedModel):
    def __init__(self,config):
        super(Capture_ITPV_ForClassification,self).__init__(config)
        self.bert=BertModel(config)
        self.cls = BertPreTrainingHeads_woITM(
            config, self.bert.embeddings.word_embeddings.weight
        )
        self.dense1=nn.Linear(config.v_hidden_size*5,config.bi_hidden_size)
