...
\title{Visual Product Search trong E-commerce:\\
Multimodal Retrieval với SCALE, Faiss HNSW và Tái xếp hạng thuộc tính}

\author{
  \IEEEauthorblockN{Trần Hải Đức}
  \IEEEauthorblockA{
    MSSV: 23127173\\
  ...
  \and
  \IEEEauthorblockN{Trần Hoàng Nam}
  \IEEEauthorblockA{
    MSSV: 23127232\\
    Trường Đại học Khoa học Tự nhiên, VNU-HCM\\
...
Mục tiêu là trả về $K$ product entry có embedding đa phương thức gần query nhất. Khác với image retrieval thuần túy, kết quả không chỉ cần giống về màu sắc hoặc texture mà còn cần phù hợp về ngữ nghĩa thương mại như siêu danh mục, danh mục, thương hiệu và các thuộc tính quyết định mua.
...
% ============================================================
\section{Truy hồi hiệu năng cao với Faiss HNSW}
% ============================================================

Faiss được chọn làm tầng truy hồi vì đây là thư viện chuyên dụng cho tìm kiếm tương tự trên vector dense, hỗ trợ cả exact search để đánh giá representation và approximate nearest-neighbor search để phục vụ catalog lớn~\cite{johnson2019faiss}. Sau khi SCALE biến mỗi listing thành một embedding đã L2-normalize, bài toán online trở thành tìm các vector có inner product/cosine similarity lớn nhất. Thực hiện phép so sánh với toàn bộ catalog ở mỗi truy vấn có chi phí tuyến tính theo số listing và nhanh chóng trở thành nút thắt khi catalog tăng quy mô. Faiss cung cấp index tối ưu cho tác vụ này, có thể lưu vector, thực hiện truy vấn batch và tách lớp truy hồi khỏi representation backbone; nhờ đó có thể thay đổi hoặc cập nhật index mà không phải huấn luyện lại SCALE.
...
% ============================================================
\begingroup
\renewcommand{\refname}{\vspace{-2\baselineskip}REFERENCES}
...
$SD, D$ & Siêu danh mục, danh mục ($D^*$ là thương hiệu đa số). \\
$I(\cdot)$ & Hàm chỉ thị; 0 nếu thiếu thành phần query. \\
$\mathrm{JCT}$ & Joint Cross-modal Transformer. \\
